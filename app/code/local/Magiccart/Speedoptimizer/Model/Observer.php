...
class Magiccart_Speedoptimizer_Model_Observer  extends Mage_Core_Block_Template
{
    protected $request;
    ...
    public function beforeSendResponse(Varien_Event_Observer $observer)
    {
        $this->helper = Mage::helper('magiccart_speedoptimizer');
    ...
    public function addLazyLoadJs( $content, $selector='img', $exclude='.loaded' )
    {
        ...
        $script = '<script type="text/javascript"> jQuery(document).ready(function($){$(' . $selector .').not("' . $exclude . '").lazyload();});</script>';
    ...
    public function addLoading($content)
    {
        ...
        $placeholder = $this->getSkinUrl('images/loader-1.gif');

        $loadingBody = $this->helper->getConfigModule('general/loading_body_placeholder');

        $loadingBody = $loadingBody ? $mediaUrl . 'magiccart/speedoptimizer/' . $loadingBody : $placeholder;
        ...
        $loadingImg  = $loadingImg ?  $mediaUrl . 'magiccart/speedoptimizer/' . $loadingImg : $placeholder;
